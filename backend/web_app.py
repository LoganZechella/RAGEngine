    ...
    async def event_generator():
        try:
            ...
            if task_id in search_tasks:
                ...
                logger.debug(f"Sent initial progress for task {task_id}: {data['progress']}%")
...
async def get_search_results(request: Request, task_id: str):
    ...
    if task["status"] != "completed":
        logger.warning(f"Task {task_id} not completed, status: {task['status']}")
...
async def api_document_list():
    """Get processed documents list as JSON for SvelteKit."""
    if not kb_api:
        raise HTTPException(status_code=503, detail="API not available")
...
async def debug_search_tasks():
    ...
    return {
        ...
        "task_details": {task_id: {
            ...
            "message": task["message"],
            "results_retrieved": task.get("results_retrieved", False)
        } for task_id, task in search_tasks.items()},
